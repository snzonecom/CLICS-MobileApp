...
class MysqlListScreen extends StatefulWidget {
  const MysqlListScreen({Key? key}) : super(key: key);
...
class _MysqlListScreenState extends State<MysqlListScreen> {
  ...
  void initState() {
    cssData =
        dictionaryData.where((data) => data['category'] == 'mySQL').toList();
    super.initState();
  }
          ...
          child: Column(
            ...
            children: [
              Column(
                children: [
                  Image.asset(
                    'lib/assets/mysql-icon.webp', // Adjust the path as needed
              ...
              Text(
                'In need for a CLI cheat sheet dictionary for MySQL? Here you go! Quickly access MysSQL command-line references on the go.',
                style: TextStyle(
                  fontSize: 16.0,
                  fontFamily: GoogleFonts.poppins().fontFamily,
                ),
