...
class WindowsListScreen extends StatefulWidget {
  const WindowsListScreen({Key? key}) : super(key: key);

  @override
  _WindowsListScreenState createState() => _WindowsListScreenState();
}

class _WindowsListScreenState extends State<WindowsListScreen> {
  late List<Map<String, dynamic>> cssData;

  @override
  void initState() {
    cssData =
        dictionaryData.where((data) => data['category'] == 'windows').toList();
    super.initState();
  }
          ...
          child: Column(
            ...
            children: [
              Column(
                children: [
                  Image.asset(
                    'lib/assets/windows-10-icon.webp', // Adjust the path as needed
              ...
              Text(
                'In need for a CLI cheat sheet dictionary for Windows? Here you go! Quickly access Windows command-line references on the go.',
                style: TextStyle(
                  fontSize: 16.0,
                  fontFamily: GoogleFonts.poppins().fontFamily,
                ),
