...
class MongodbListScreen extends StatefulWidget {
  const MongodbListScreen({Key? key}) : super(key: key);

  @override
  _MongodbListScreenState createState() => _MongodbListScreenState();
}

class _MongodbListScreenState extends State<MongodbListScreen> {
  late List<Map<String, dynamic>> cssData;

  @override
  void initState() {
    cssData =
        dictionaryData.where((data) => data['category'] == 'mongoDB').toList();
    super.initState();
  }
          ...
          child: Column(
            ...
            children: [
              Column(
                children: [
                  Image.asset(
                    'lib/assets/mongodb-icon.webp', // Adjust the path as needed
              ...
              Text(
                'In need for a CLI cheat sheet dictionary for MongoDB? Here you go! Quickly access MongoDB command-line references on the go.',
                style: TextStyle(
                  fontSize: 16.0,
                  fontFamily: GoogleFonts.poppins().fontFamily,
                ),
